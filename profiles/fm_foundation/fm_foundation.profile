...
function fm_foundation_form_install_configure_form_alter(&$form, $form_state) {
  // Add profile as site name.
  $form['site_information']['site_name']['#default_value'] = 'FM Foundation';
	//On submit, execute foundation_profile_install function
	$form['#submit'][] = '_fm_foundation_profile_install_configure_form_submit';
}

function _fm_foundation_profile_install_configure_form_submit($form, &$form_state) {
	//Instead of typical installation, loading preconfigured db included with installation profile	
	global $user;
  $sql = dirname(__FILE__).'/db.sql';
  $count = foundation_update_db($sql);
...
function foundation_update_db($sql_file){
  global $databases;
...
function fm_foundation_install_tasks_alter(&$tasks, $install_state) {
  // Replace the install_finished installation task 
  $tasks['install_finished']['function'] = 'fm_foundation_install_finished';
}

function fm_foundation_install_finished(&$install_state) {
  drupal_set_title(st('@drupal installation complete', array('@drupal' => drupal_install_profile_distribution_name())), PASS_THROUGH);
  $messages = drupal_set_message();
  $output = '<p>' . st('Congratulations, you installed Foundation!', array('@drupal' => drupal_install_profile_distribution_name())) . '</p>';
